...
scatterplot(parenthood$baby.sleep, parenthood$dan.grump)
...
scatterplot(parenthood$baby.sleep, parenthood$dan.sleep)
```
...
```{r}
correlate(mtcars$hp, mtcars$mpg)
```
```{r}
scatterplot(mtcars$hp, mtcars$mpg, smooth = FALSE)
```
```{r}
cov(mtcars$hp, mtcars$mpg)
```
```{r}
scatterplot(mtcars$wt, mtcars$mpg, smooth = FALSE)
correlate(mtcars$wt, mtcars$mpg)
```
