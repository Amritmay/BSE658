...
sub = "Fibonacci numbers and their index values",
xlab = "Index",
ylab = "Fibonacci numbers" )
```
...
font.main = 6,                          # plain text for title
cex.main = 2,                           # normal size for title
font.axis = 1,                          # bold text for numbering
col.lab = "salmon3",                    # grey color for labels
col.main = "seagreen",
...
barplot( height = freq, names.arg = teams, main = "AFL finalists", 
         #sub = " Number of times a team reached finals", 
         col = "red2", las = 2,
         col.axis = "blue2") #Adding labels to the bar plot
```
```{r}
...
barplot( height = freq,
         ...
         density = 20,                       # shade the bars
         angle = 90 )                        # shading lines angle
...
hist( x = afl.margins, breaks = 4 )
hist( x = afl.margins, breaks = 5 ) 
...
```{r}
A = rnorm(n = 1000, mean = 100, sd = 50)
a = hist(A)

B = 0;
for (i in 1:10) {
  B.n = rnorm(n = 100, mean = 100, sd = 50)
  B[i] = mean(B.n)
}
b = hist (B)
```
